...
若完整候選 evidence 的格式可讀，但 Task 驗證拒絕，`task-finish` 提示會附 `check_evidence`：check ID、evidence 路徑、`recorded_status` 與閱讀提示。`failed` 表示該次執行失敗，先讀 evidence 的 `stdout`／`stderr` 與執行旗標，再決定修正方式；`passed` 只代表該次執行通過，仍須依原 blocker 查核本輪有效性。只有通過前置驗證、提交與乾淨工作目錄檢查後，確認內容不匹配的紀錄才提示重新驗證。缺檢查仍先提示補齊紀錄；未提交或工作目錄不乾淨的 blocker 不會被解讀為 evidence 過期；證據損壞、讀取失敗不猜測測試結果。提示不複製測試輸出、不代判根因，也不自動重跑。

...
本文件是 Readiness 判準的唯一來源。其他 workflow 使用本文件的判定結果控制流程，不另行定義 `ready`／`blocked` 的成立條件。
...
每個欄位由 AI 依內容選擇呈現方式：單一簡短結論可用短句；多個彼此獨立、沒有共同比較欄位的事項使用列點；多個事項具有相同屬性，且表格能讓對應或比較更清楚時，可用 Markdown 表格，例如「情況／回應」或「阻塞事項／影響／解除條件」。同一欄位內可混用不同格式，不要求每份摘要都有表格。

七個欄位仍各自保留，不因使用表格而合併或改變內容分類。同一事項的理由、限制或證據跟隨該事項；使用表格時放在對應列，或在表外明確註明所對應的事項，不另拆成無關的獨立項目。不得為填滿表格而補出未確認的內容。

沒有內容的欄位仍保留標題並寫「無」；尚未釐清不能冒充「無」。狀態行沿用上述名稱和值，只輸出當下有效的狀態。

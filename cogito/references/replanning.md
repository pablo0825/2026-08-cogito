...
所有可核准的 RP proposal 都必須包含 `start_artifact` 與 `start_artifact_hash`。缺少 immutable Start artifact 的舊 proposal 可以保留作為歷史證據，但不能再核准或執行 handoff；應撤銷該 RP，再從目前有效基線建立新 proposal。

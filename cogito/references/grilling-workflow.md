...
- 「跳過這題」：第一次跳過必要問題時，暫時移出本輪提問並保留未決，先處理其他可獨立回答的必要問題，完成後再回訪該題，不立即重問。回訪後若再次跳過，目前不再主動追問該題，保留阻塞並依停止條件處理。跳過不代表問題不再必要，不自動轉為 `defer`、`prune` 或接受 AI 預設。
...
- 依「跳過這題」指令回訪後仍被跳過的必要問題，目前不視為可有效繼續追問的事項；其他可獨立進行的必要問答或查證仍須先處理。若已無其他可有效釐清的必要事項，依下述條件產出 `blocked` 摘要，記錄缺少的決策與恢復條件，不以反覆回訪取代摘要。

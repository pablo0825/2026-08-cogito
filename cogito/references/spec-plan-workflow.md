...
全部成立時直接進入「核准」。package 對話若已中斷、current drafts 是否仍為使用者看到的 exact package 無法證明，先重新產生摘要再等待核准，不靠猜測續用舊內容。任一條件不成立時依入口分類轉入適當 revision；證據不足時停止並指出缺少的核准資格，不以重新 Grilling 代替可查證的 Git／文件事實。

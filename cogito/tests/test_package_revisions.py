    ...
    def test_candidate_revisions_require_latest_approval_and_freeze_after_approval(self) -> None:
        for kind in ("feature", "maintenance", "documentation"):
            with self.subTest(kind=kind):
                ...
                self.assertNotIn("next", initial)
                initial_next = json.loads(self.invoke(repo, "next", "--run-id", run_id).stdout)["data"]
                self.assertEqual(initial_next["candidate_package_hash"], package_hash(first))
                ...
                updated_next = json.loads(self.invoke(repo, "next", "--run-id", run_id).stdout)["data"]
                self.assertEqual(updated_next["candidate_package_hash"], package_hash(revised))
                ...
                replayed_next = json.loads(self.invoke(repo, "next", "--run-id", run_id).stdout)["data"]
                self.assertEqual(replayed_next["candidate_package_hash"], package_hash(revised))

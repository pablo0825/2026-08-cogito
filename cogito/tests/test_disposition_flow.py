...
class DispositionFlowTests(GitTestCase):
    ...
    def test_cancel_then_retain_requires_fresh_explicit_acceptance(self):
        ...
        self.assertTrue(self.cli(repo, 'status')['resolution']['human_accepted'])
        self.assertEqual(source.load()['state'], 'cancelled')
    ...
    def test_retention_followup_acceptance_can_return_for_correction(self):
        ...
        next_hint = followup.next_action()
        self.assertEqual(next_hint['next_action'], 'complete-disposition')
        self.assertNotIn('report', next_hint)
        self.assertEqual(next_hint['report_query']['operation'], 'report')
        self.assertEqual(next_hint['cleanup']['status'], 'pending')
        self.assertTrue(next_hint['cleanup']['observation_only'])
        self.assertNotIn('operations', next_hint)
        self.assertTrue(any('active disposition' in row['reason'] for row in next_hint['cleanup']['retained']))
        self.assertEqual(followup.completion_report()['status'], 'accepted')

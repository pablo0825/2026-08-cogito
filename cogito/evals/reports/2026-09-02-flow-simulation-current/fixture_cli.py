...
class FeatureMultiSliceEndToEndTests(GitTestCase):
    ...
    def _run_feature(
        ...
        with persistent_repo() as directory:
            repo = Path(directory)
            ...
            if SCENARIO == "human-gate":
                draft["human_gate"]["predicates"] = [{"id": "HA-001", "applicable": True}]
            if workflow is not None:
                draft["limits"]["transient_retries"] = workflow["limits"]["transient_retries"]
            store = CliStore(repo, run_id, workflow)
            store.create(KIND)
            ...
            if SCENARIO == "integration-outside":
                git(repo, "merge", "--no-ff", "--no-commit", "codex/fs-a")
                (repo / "unrelated.txt").write_text("unapproved integration change\n")
                git(repo, "add", "unrelated.txt")
                git(repo, "commit", "-qm", "integrate FS-A with unrelated change")
            else:
            ...
            if SCENARIO == "human-gate":
                ...
                state = store.call("human-approve")
            ...
            result_path.write_text(json.dumps({
                ...
                "amendments": [], "human_gate": {"required": SCENARIO == "human-gate", "outcome": "approved" if SCENARIO == "human-gate" else "not-required"},
                "remaining_risks": [],
            }))
